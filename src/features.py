...
def extract_features(img_dir, result_dir, input_wh):
    ...
    for fname in image_fnames:
        ...
        cv2.imwrite(os.path.join(result_dir, f'{fname}_joints_vis.png'), joints2D_vis)
        cv2.imwrite(os.path.join(result_dir, f'{fname}_joints.png'), joints2D)
        cv2.imwrite(os.path.join(result_dir, f'{fname}_silh_vis.png'), silhouette_vis)
        cv2.imwrite(os.path.join(result_dir, f'{fname}_silh.png'), silhouette)
        
        
def count_human_pixels(seg_img):
    return np.count_nonzero(np.count_nonzero(seg_img, axis=2))


def get_height_in_image(seg_img):
    # NOTE: Assuming that
    return np.count_nonzero(np.count_nonzero(np.count_nonzero(seg_img, axis=2), axis=1))


if __name__ == '__main__':
    extract_features(IMG_DIR, RESULT_DIR, INPUT_WH)
